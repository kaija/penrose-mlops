...
* Usages of the `run-notebook` Action should be replaced by [installing the Databricks CLI](https://github.com/databricks/databricks-cli#installation)
  ...
  ([docs](https://docs.gcp.databricks.com/dev-tools/cli/runs-cli.html#submit-a-one-time-run)).
...
* Be a Databricks workspace admin in the staging and prod workspaces. Verify that you're an admin by viewing the
  [staging workspace admin console](https://your-staging-workspace.gcp.databricks.com#setting/accounts) and
...
* Configure a Databricks CLI profile for your staging workspace by running
  ``databricks configure --token --profile "penrose-staging" --host https://your-staging-workspace.gcp.databricks.com``, 
  which will prompt you for a REST API token
* Create a [Databricks REST API token](https://docs.gcp.databricks.com/dev-tools/api/latest/authentication.html#generate-a-personal-access-token)
  in the staging workspace ([link](https://your-staging-workspace.gcp.databricks.com#setting/account))
  and paste the value into the prompt.
* Configure a Databricks CLI for your prod workspace by running ``databricks configure --token --profile "penrose-prod" --host https://6837671528024691.1.gcp.databricks.com``
...
Ensure a group named `penrose-service-principals` exists in the staging and prod workspace, e.g.
by checking for the group in the [staging workspace admin console](https://your-staging-workspace.gcp.databricks.com#setting/accounts/groups) and
...
Then, grant the `penrose-service-principals` group [token usage permissions](https://docs.gcp.databricks.com/administration-guide/access-control/tokens.html#manage-token-permissions-using-the-admin-console)
