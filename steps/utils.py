...
def get_deployed_model_stage_for_env(env):
    ...
    # (https://docs.gcp.databricks.com/applications/machine-learning/manage-model-lifecycle/index.html#transition-a-model-stage).
...
def get_model_name(env, test_mode=False):
    ...
    if env == "dev" or test_mode:
        ...
        return f"penrose-model{resource_name_suffix}"
    else:
        # Read ml model name from databricks-config
        return _get_ml_config_value(env, "penrose_model_name")
